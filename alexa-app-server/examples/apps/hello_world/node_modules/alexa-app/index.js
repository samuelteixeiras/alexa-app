  ...
  this.response = {
   ...
   this.test = function(stop,bus) {
    ...
    jsdom.env(
      ...
      function (err, window) {
        var str = "there have been" + window.$("a").length  + "io.js releases!";
        console.log(str);

        if (typeof _self.response.response.outputSpeech == "undefined") {
        _self.response.response.outputSpeech = {
          "type": "SSML",
          "ssml": SSML.fromStr(str)
        };
      } else {
        // append str to the current outputSpeech, stripping the out speak tag
        _self.response.response.outputSpeech.ssml = SSML.fromStr(str, _self.response.response.outputSpeech.ssml);
      }
      console.log(_self);
      return _self;
